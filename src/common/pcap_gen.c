...
static const char *get_clean_query(uint32_t *seed) {
    ...
    unsigned int idx = simple_rand(seed) % 10;
    return queries[idx];
}
...
static int write_query_packet(FILE *fp, const char *query, time_t ts_sec, uint32_t ts_usec) {
    ...
    uint32_t len_be = (uint32_t)(query_len + 1 + 4);  /* query + null + 4-byte length field */
    msg[1] = (unsigned char)((len_be >> 24) & 0xFF);
    msg[2] = (unsigned char)((len_be >> 16) & 0xFF);
    msg[3] = (unsigned char)((len_be >> 8) & 0xFF);
    msg[4] = (unsigned char)(len_be & 0xFF);
    ...
    /* Write packet header */
    pkt_hdr.ts_sec = (uint32_t)ts_sec;
    pkt_hdr.ts_usec = ts_usec;
    pkt_hdr.incl_len = (uint32_t)msg_len;
    pkt_hdr.orig_len = (uint32_t)msg_len;
...
int pcap_gen_write(const char *filename, const pcap_gen_opts_t *opts) {
    ...
    for (int i = 0; i < opts->num_clean; i++) {
        ...
        if (write_query_packet(fp, query, ts_sec, (uint32_t)i * 100000) < 0) {  /* 100ms intervals */
    ...
    for (int i = 0; i < opts->num_injection; i++) {
        ...
        if (write_query_packet(fp, query, ts_sec, (uint32_t)(opts->num_clean + i) * 100000) < 0) {
